...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-4 col-sm-4 col-xs-12 ">
            <div class="card">
                <div class="card-header text-white shadow text-center" style="background-color: #4d4d4d">
                    <h5>Questions</h5>
                </div>
                    ...
                    <table class="table table-bordered shadow text-center bg-info table-dark table-small">
                        ...
                        <tbody>
                            @foreach($queans as $row)
                                <tr><td>{!! $row['que'] !!}</td><td><a href="{{ custom_url('teacher/event/'.$id.'/que/'.$row['id']) }}" class="btn btn-primary btn-sm">Edit</a><form method="POST" action="{{ custom_url('teacher/event/'.$id.'/delete/que/'.$row['id']) }}"> {{ csrf_field() }} <button class="btn btn-danger btn-sm" type="submit" onclick="return confirm('Are you sure to delete?')">Delete</button> </form></td></tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
