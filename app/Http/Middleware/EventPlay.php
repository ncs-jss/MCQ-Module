    ...
    public function handle($request, Closure $next)
    {
        if (session()->has('event')) {
            ...
            if (!session()->has('eventcheck')) {
                $reqs = Req::select('eventid', 'status', 'que', 'start')
                    ->where('status', 1)
                    ->where('userid', Auth::id())
                    ->whereNotNull('que')
                    ->get()
                    ->toArray();
                $events = Event::select('id', 'start', 'end', 'isactive', 'quedisplay', 'duration')
                    ->whereIn('id', array_column($reqs, 'eventid'))
                    ->get()
                    ->toArray();
                $data = [];
                foreach ($reqs as $req) {
                    $key = array_search($req['eventid'], array_column($events, 'id'));
                    if ($events[$key]['isactive'] == 1
                        && $events[$key]['start'] <= date('Y-m-d H:i:s')
                        && $events[$key]['end'] >= date('Y-m-d H:i:s')) {
                        if (strtotime($req['start']." + ".$events[$key]['duration']." minute")
